...
<head>
    ...
    <title>{{ config('app.name') }} · Soluciones CNC</title>
...
<section class="relative hero-bg text-white overflow-hidden min-h-screen flex flex-col">
                ...
                <svg class="logo-sun" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg" fill="white">
                    ...
                    @for($i = 0; $i < 16; $i++)
                        <rect x="38" y="2" width="4" height="14"
                              transform="rotate({{ $i * 22.5 }} 40 40)" rx="1"/>
                    @endfor
                    {{-- Globo central --}}
    ...
    {{-- Indicador scroll (chevron abajo) --}}
                ...
                @foreach($carouselBrands as $b)
                    <div class="brand-card flex-shrink-0 w-56 h-40 bg-white rounded-2xl shadow-md
                        ...
                        @if(!empty($b->logo_path))
                            <img src="{{ asset('storage/'.$b->logo_path) }}"
                                 alt="{{ $b->name }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach($machines as $m)
                <article class="bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-xl transition">
                    @if($m->image_path)
                        <img src="{{ asset('storage/'.$m->image_path) }}" alt="{{ $m->name }}" class="w-full h-48 object-cover">
                    @else
                        <div class="w-full h-48 bg-slate-100 flex items-center justify-center text-slate-300 text-sm">Sin imagen</div>
                    @endif
                    <div class="p-5">
                        <p class="text-xs text-sky-600 font-semibold uppercase tracking-wider">{{ $m->brand?->name }}</p>
                        <h3 class="font-bold text-slate-900 mt-1">{{ $m->name }}</h3>
                        <p class="text-sm text-slate-600 mt-2">{{ Str::limit($m->description, 90) }}</p>
                    </div>
                </article>
            @endforeach
        </div>
